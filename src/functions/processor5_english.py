...
def convert_example_to_features(example, max_seq_length, is_training, prem_max_sentence_length, hypo_max_sentence_length, language):
...
def convert_examples_to_features(
    ...
    if return_dataset == "pt":
        ...
        if not is_training:
            ...
            all_annotator_labels = torch.tensor([f.annotator_labels for f in features], dtype=torch.long)
    ...
    def _create_examples(self, input_data, set_type, data_file):
        ...
        for entry in tqdm(input_data):
            ...
            if is_training:
                ...
                annotator_labels = entry["annotator_labels"]
            else:
...
class NLIExample(object):
    def __init__(
        ...
        for i,annotator_label in enumerate(annotator_labels):
            if annotator_label in label_dict.keys():
                annotator_labels[i] = label_dict[annotator_label]
        ...
        merge_index = [[],[]]
        merge_word = [[],[]]
        if len(self.premise_merge)==0: self.premise_merge = [self.premise.strip(),
                                                             [i for i, _ in enumerate(self.premise.strip().split())]]
        for merge in self.premise_merge:
            if merge[1] != []: merge_index[0].append(merge[1])

        if len(self.hypothesis_merge) == 0: self.hypothesis_merge = [self.hypothesis.strip(),
                                                               [i for i, _ in enumerate(self.hypothesis.strip().split())]]
        for merge in self.hypothesis_merge:
            if merge[1] != []: merge_index[1].append(merge[1])

        # 구문구조 종류
        depend2idx = {"None":0}; idx2depend ={0:"None"}
        for depend in ['ROOT', 'acl', 'acomp', 'advcl', 'advmod', 'agent', 'amod', 'appos', 'attr', 'aux', 'auxpass',
        ...
        other_tag = ['ROOT', 'agent', 'attr', 'case', 'cc', 'compound', 'csubj', 'csubjpass',
                     'dative', 'dep', 'dobj', 'expl', 'intj', 'mark', 'nsubj', 'nsubjpass', 'oprd',
                     'parataxis', 'pobj', 'preconj', 'predet', 'prt', 'punct']


        if len(self.premise_koala) == 0: merge_word[0].append([])
        elif ([words for words in self.premise_koala if words[2][0] != words[2][1]] == []): merge_word[0].append([])
        else:
            for words in self.premise_koala:
                if words[2][0] != words[2][1]:
                    if not [merge_index[0].index(words[2][0]), merge_index[0].index(words[2][1])] in [merge_word[:-1] for merge_word in merge_word[0]]:
                        merge_word[0].append([merge_index[0].index(words[2][0]), merge_index[0].index(words[2][1]),depend2idx[words[1]]])
                    else:
                        idx = [merge_w[:-1] for merge_w in merge_word[0]].index([merge_index[0].index(words[2][0]), merge_index[0].index(words[2][1])])
                        tag = idx2depend[[merge_w[-1] for merge_w in merge_word[0]][idx]]
                        if (words[1] in other_tag) and(tag not in other_tag):
                            merge_word[0][idx][2] = depend2idx[words[1]]


        if ([words for words in self.hypothesis_koala if words[2][0] != words[2][1]] == []): merge_word[1].append([])
        elif ([words for words in self.hypothesis_koala if words[2][0] != words[2][1]] == []): merge_word[1].append([])
        else:
            for words in self.hypothesis_koala:
                if words[2][0] != words[2][1]:
                    if not [merge_index[1].index(words[2][0]), merge_index[1].index(words[2][1])] in [merge_word[:-1] for merge_word in merge_word[1]]:
                        merge_word[1].append([merge_index[1].index(words[2][0]), merge_index[1].index(words[2][1]),depend2idx[words[1]]])
                    else:
                        idx = [merge_w[:-1] for merge_w in merge_word[1]].index([merge_index[1].index(words[2][0]), merge_index[1].index(words[2][1])])
                        tag = idx2depend[[merge_w[-1] for merge_w in merge_word[1]][idx]]
                        if (words[1] in other_tag) and(tag not in other_tag):
                            merge_word[1][idx][2] = depend2idx[words[1]]
