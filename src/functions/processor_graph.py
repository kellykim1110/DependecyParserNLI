...
class KLUE_NLIProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, filename=None, depend_embedding = None):
        ...
        return self._create_examples(input_data, depend_embedding, 'train')
    ...
    def get_dev_examples(self, data_dir, filename=None, depend_embedding = None):
        ...
        with open(
            ...
            input_data = json.load(reader)
        return self._create_examples(input_data, depend_embedding, "dev")
    ...
    def _create_examples(self, input_data, depend_embedding, set_type):
        ...
        for entry in tqdm(input_data):
            ...
            example = KLUE_NLIExample(
                ...
                gold_label=gold_label,
                label=label,
                depend_embedding = depend_embedding,
            )
...
class KLUE_NLIExample(object):
    def __init__(
        ...
        prem_MG = nx.MultiGraph();  hypo_MG = nx.MultiGraph();
        prem_GG = nx.Graph(); hypo_GG = nx.Graph();

        prem_back_words = set(word[1] for word in merge_word[0])
        hypo_back_words = set(word[1] for word in merge_word[1])

        prem_MG.add_weighted_edges_from([(word[0], word[1], depend_embedding[word[2]]) for word in merge_word[0]])
        hypo_MG.add_weighted_edges_from([(word[0], word[1], depend_embedding[word[2]]) for word in merge_word[1]])

        for n, nbrs in prem_MG.adjacency():
            for nbr, edict in nbrs.items():
                minvalue = min([d['weight'] for d in edict.values()])
                prem_GG.add_edge(n, nbr, weight=minvalue)

        for n, nbrs in hypo_MG.adjacency():
            for nbr, edict in nbrs.items():
                minvalue = min([d['weight'] for d in edict.values()])
                hypo_GG.add_edge(n, nbr, weight=minvalue)

        for G1 in back_words:
            sp = dict(nx.all_pairs_shortest_path(GG))
            print(sp[G1])
        exit()
