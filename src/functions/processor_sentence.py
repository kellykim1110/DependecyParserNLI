...
def klue_convert_example_to_features(example, max_seq_length, is_training, language):
    ...
    features=[]
    for sen in[example.premise, example.hypothesis]:
        ...
        if tokenizer.padding_side == "right":
            p_mask[:len(all_doc_tokens["premise"])] = 0
        else:
            p_mask[-len(all_doc_tokens["premise"]): ] = 0
        ...
        p_mask[cls_index] = 0
        features.append(KLUE_NLIFeatures(
            input_ids,
            attention_mask,
            token_type_ids,
            #position_ids,
...
def klue_convert_examples_to_features(
    ...
    new_premise_features, new_hypothesis_features = [], []
    ...
    ):  # example_features = (premise_features, hypothesis_features)
        if not example_features:
            continue

        example_features[0].example_index = example_index
        example_features[1].example_index = example_index
        new_premise_features.append(example_features[0])
        new_hypothesis_features.append(example_features[1])
        example_index += 1
    ...
    if return_dataset == "pt":
        ...
        all_prem_input_ids = torch.tensor([f.input_ids for f in new_premise_features], dtype=torch.long)
        all_prem_attention_masks = torch.tensor([f.attention_mask for f in new_premise_features], dtype=torch.long)

        all_prem_token_length = torch.tensor([len(f.tokens) for f in new_premise_features], dtype=torch.long)

        ## RoBERTa doesn’t have token_type_ids, you don’t need to indicate which token belongs to which segment.
        if language == "electra":
            all_prem_token_type_ids = torch.tensor([f.token_type_ids for f in new_premise_features], dtype=torch.long)
        # all_prem_position_ids = torch.tensor([f.#position_ids for f in new_premise_features], dtype=torch.long)

        all_prem_cls_index = torch.tensor([f.cls_index for f in new_premise_features], dtype=torch.long)
        all_prem_p_mask = torch.tensor([f.p_mask for f in new_premise_features], dtype=torch.float)

        all_example_indices = torch.tensor([f.example_index for f in new_premise_features], dtype=torch.long)
        all_prem_feature_index = torch.arange(all_prem_input_ids.size(0), dtype=torch.long)  # 전체 feature의 개별 index

        # Convert to Tensors and build dataset
        all_hypo_input_ids = torch.tensor([f.input_ids for f in new_hypothesis_features], dtype=torch.long)
        all_hypo_attention_masks = torch.tensor([f.attention_mask for f in new_hypothesis_features], dtype=torch.long)

        all_hypo_token_length = torch.tensor([len(f.tokens) for f in new_hypothesis_features], dtype=torch.long)

        ## RoBERTa doesn’t have token_type_ids, you don’t need to indicate which token belongs to which segment.
        if language == "electra":
            all_hypo_token_type_ids = torch.tensor([f.token_type_ids for f in new_hypothesis_features], dtype=torch.long)
        # all_hypo_position_ids = torch.tensor([f.#position_ids for f in new_hypothesis_features], dtype=torch.long)

        all_hypo_cls_index = torch.tensor([f.cls_index for f in new_hypothesis_features], dtype=torch.long)
        all_hypo_p_mask = torch.tensor([f.p_mask for f in new_hypothesis_features], dtype=torch.float)

        all_hypo_feature_index = torch.arange(all_hypo_input_ids.size(0), dtype=torch.long)  # 전체 feature의 개별 index
        ...
        if not is_training:
            ...
            if language == "electra":
                dataset = TensorDataset(
                    all_prem_input_ids,
                    all_prem_attention_masks,
                    all_prem_token_type_ids,
                    #all_prem_position_ids,
                    all_prem_token_length,
                    all_prem_cls_index,
                    all_prem_p_mask, # encoder_attention_mask
                    all_prem_feature_index,
                    all_hypo_feature_index,
                    all_hypo_p_mask,
                    all_hypo_cls_index,
                    all_hypo_token_length,
                    #all_hypo_position_ids,
                    all_hypo_token_type_ids,
                    all_hypo_attention_masks,
                    all_hypo_input_ids,
                )
            else:
                dataset = TensorDataset(
            ...
            all_labels = torch.tensor([f.label for f in new_hypothesis_features], dtype=torch.long)
        ...
        return [new_premise_features, new_hypothesis_features], dataset
    else:
        return [new_premise_features, new_hypothesis_features]
