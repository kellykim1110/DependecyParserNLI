...
from src.functions.metric import get_score
...
def train(args, model, tokenizer, logger):
    ...
    for epoch in mb:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs_list.append("token_length")
            inputs = dict()

            for n, input in enumerate(inputs_list):
                inputs["prem_"+str(input)] = batch[n]
                inputs["hypo_" + str(input)] = batch[-n-1]

            inputs_list.append("labels")
            inputs["labels"] = batch[len(inputs_list)-1]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                #if (float(results["accuracy"]) >= max_acc):
    ...
    for i,(g,p) in enumerate(zip(gold_labels, pred_labels)):
        if g != p:
            for k,v in zip(out.keys(),[examples[i].premise, examples[i].hypothesis, idx2label[g], idx2label[p]]):
                out[k].append(v)
    df = pd.DataFrame(out)
    df.to_csv(os.path.join(output_dir, "test_result_{}_incorrect.csv".format(eval_file_name)), index=False)
