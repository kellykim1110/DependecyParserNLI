...
def merge_tag(inf_dir, outf_dir):
    ...
    for id, data in tqdm(enumerate(datas)):
        output = [];output2 = [];
        # {"origin": text, "koala":root, words:[[word, tag, idx], [], ... ]}
        ...
        for texts in texts_list:
            ...
            MOD = []; AJT = []; CMP = [];
            np_cnj_list = []
            #print(texts["koala"]["words"])

            # 전처리
            ## [['어떤 방에서도', '금지됩니다.'], ['NP', 'AJT'], [[0, 1], [3]]], [['방에서도', '흡연은'], ['NP', 'MOD'], [[1], [2]]]와 같은 경우
            for i, koala in enumerate(texts["koala"]["words"]):
                for j, other in enumerate(texts["koala"]["words"]):
                    if (texts["koala"]["words"][i][2][0] != other[2][0]) and (set(texts["koala"]["words"][i][2][0]+other[2][0]) == set(other[2][0])):
                        texts["koala"]["words"][i][0][0] = other[0][0]
                        texts["koala"]["words"][i][2][0] = other[2][0]
                    if (texts["koala"]["words"][i][2][0] != other[2][1]) and (set(texts["koala"]["words"][i][2][0]+other[2][1]) == set(other[2][1])):
                        texts["koala"]["words"][i][0][0] = other[0][1]
                        texts["koala"]["words"][i][2][0] = other[2][1]
                    if (texts["koala"]["words"][i][2][1] != other[2][0]) and (set(texts["koala"]["words"][i][2][1]+other[2][0]) == set(other[2][0])):
                        texts["koala"]["words"][i][0][1] = other[0][0]
                        texts["koala"]["words"][i][2][1] = other[2][0]
                    if (texts["koala"]["words"][i][2][1] != other[2][1]) and (set(texts["koala"]["words"][i][2][1]+other[2][1]) == set(other[2][1])):
                        texts["koala"]["words"][i][0][1] = other[0][1]
                        texts["koala"]["words"][i][2][1] = other[2][1]
            #print(texts["koala"]["words"])
            for i, koala in enumerate(texts["koala"]["words"]):
                #print(koala)
                tag = koala[1]
            ...
            for tag_idx, _ in enumerate(tag_list):
                ...
                if (len(np_cnj_list) != 0):
                    ...
                    for cnj_koala in np_cnj_list:
                        if cnj_koala in tag_list[tag_idx]:
                            for ttag_list in tag_list[tag_idx]:
                                if (len(set(cnj_koala[2][0]).intersection(set(ttag_list[2][0]))) != 0):
                                    new_koala = [[cnj_koala[0][1], ttag_list[0][1]], [cnj_koala[1][1], ttag_list[1][1]],
                                                 [cnj_koala[1][1], ttag_list[1][1]]]
                                    tag_list[tag_idx].append(new_koala)
                                if (len(set(cnj_koala[2][1]).intersection(set(ttag_list[2][0]))) != 0):
                                    new_koala = [[cnj_koala[0][0], ttag_list[0][1]], [cnj_koala[1][0], ttag_list[1][1]],
                                                 [cnj_koala[1][0], ttag_list[1][1]]]
                                    tag_list[tag_idx].append(new_koala)
                                if (len(set(cnj_koala[2][0]).intersection(set(ttag_list[2][1]))) != 0):
                                    new_koala = [[ttag_list[0][0], cnj_koala[0][1]], [ttag_list[1][0], cnj_koala[1][1]],
                                                 [ttag_list[1][0], cnj_koala[1][1]]]
                                    tag_list[tag_idx].append(new_koala)
                                if (len(set(cnj_koala[2][1]).intersection(set(ttag_list[2][1]))) != 0):
                                    new_koala = [[ttag_list[0][0], cnj_koala[0][0]], [ttag_list[1][0], cnj_koala[1][0]],
                                                 [ttag_list[1][0], cnj_koala[1][0]]]
                                    tag_list[tag_idx].append(new_koala)
                    #print("tag_list: " + str(tag_list[tag_idx]))
    ...
    for i, (data, merge1, merge2) in enumerate(zip(datas, outputs, outputs2)):
        datas[i]["premise"]["merge"] = {"origin": merge2[0], "koala": merge1[0]}
        datas[i]["hypothesis"]["merge"] = {"origin": merge2[1], "koala": merge1[1]}
...
if __name__ =='__main__':
    ...
    inf_dirs = ["./../../data/koala/koala_ver1_klue_nli_train.json", "./../../data/koala/koala_ver1_klue_nli_dev.json"]
    outf_dirs = ["./../../data/merge/merge_3_klue_nli_train.json", "./../../data/merge/merge_3_klue_nli_dev.json"]

    for inf_dir, outf_dir in zip(inf_dirs, outf_dirs):
        merge_tag(inf_dir, outf_dir)
